...
max_tokens=32768
# 换成自己要infer的model
save_name=llama-v0.2_reinforce-v0.1_mix-data-v4_step60_local
...
datasets=('college_math')
# 'gaokao_math_cloze' 'gaokao2023en' 'gaokao2024' 'omni_math'
task=zs
base_dir=$(pwd)
...
for i in "${!datasets[@]}"; do
    ...
    for id in $(seq 0 $((worker_num - 1))); do
        if [[ $id -eq $worker_id ]]; then
            ...
            for cd in $(seq 0 $((cuda_num/4 - 1))); do
            ...
            memory_threshold=5000 # Memory threshold in MiB
            ...
            sleep 2 # Additional sleep after all GPUs are free
        fi
    done
done
